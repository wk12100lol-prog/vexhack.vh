...
# AES-256 encryption (optional – requires cryptography package)
_HAS_CRYPTO = False
try:
...
class CMPCompressor:
    ...
    @staticmethod
    def compress(data, passes=3):
        if not data: return b""
        current = data; all_mappings = []
        for _ in range(passes):
            mapping, current = CMPCompressor._bpe_build(current, 16)
...
class VHCompressor:
    ...
    def compress(data):
        if not data: return b""
        current = data; all_mappings = []
        passes = max(2, min(8, len(data) // 200))
        for _ in range(passes):
            mapping, current = VHCompressor._bpe_build_23(current, 48)
            if not mapping: break
...
def _read_archive_header(data, v1_magic, v2_magic):
    """Returns (version, num_files, flags, salt, offset) or None."""
    if len(data) >= 9 and data[:4] == v1_magic:
        # v1: [MAGIC:4][num:1][reserved:4] = 9
        num_files = data[4]
        return (1, num_files, 0, None, 9)
    if len(data) >= 7 and data[:4] == v2_magic:
        # v2: [MAGIC:4][ver:1][num:1][flags:1] = 7 (+ optional salt)
        version = data[4]
        num_files = data[5]
        flags = data[6]
        offset = 7
...
def _read_file_entry_v1(data, offset):
    """Returns (name, orig_size, comp_size, entry_end) or None."""
    ...
    return (name, orig_size, comp_size, offset + comp_size)

def _read_file_entry_v2(data, offset, flags, key=None):
    """Returns (name, orig_size, comp_size, comp_data, entry_end) or None."""
    if offset + 2 > len(data): return None
    name_len = struct.unpack("<H", data[offset:offset+2])[0]; offset += 2
    if offset + name_len > len(data): return None
    name = data[offset:offset+name_len].decode("utf-8"); offset += name_len
...
class CMPArchive:
    ...
    @staticmethod
    def pack(files, output_path, password=None, use_crc=True):
        file_entries = []
        all_data = bytearray()
        key, salt = None, None
        ...
        for name, content in files:
            ...
            compressed = CMPCompressor.compress(bytes(content))
    ...
    def pack_raw(raw_files, output_path, password=None, use_crc=True):
        """Pack using pre-compressed data: list of (name, orig_content, compressed_bytes)."""
        file_entries = []
        all_data = bytearray()
        key, salt = None, None
        if password and _HAS_CRYPTO:
    ...
    @staticmethod
    def unpack(archive_path, output_dir, password=None):
        ...
        for _ in range(num_files):
            if version == 1:
                r = _read_file_entry_v1(data, offset)
                if not r: break
                name, orig_size, comp_size, offset = r
                comp_data = data[offset - comp_size:offset]
    ...
    def list_files(archive_path, password=None):
        """Return list of (name, orig_size, comp_size, has_crc, is_encrypted, crc_value) without extracting."""
        with open(archive_path, "rb") as f:
            data = f.read()
        hdr = _read_archive_header(data, CMPCompressor.MAGIC, CMPArchive.V2_MAGIC)
        if not hdr: return None
    ...
    @staticmethod
    def list_files(archive_path, password=None):
        """Return list of (name, orig_size, comp_size, has_crc, is_encrypted) without extracting."""
        with open(archive_path, "rb") as f:
            data = f.read()
        hdr = _read_archive_header(data, CMPCompressor.MAGIC, CMPArchive.V2_MAGIC)
        if not hdr: return None
        ...
        for _ in range(num_files):
            if version == 1:
                ...
                if not r: break
                name, orig_size, comp_size, offset = r
                result.append((name, orig_size, comp_size, False, False, False))
            else:
...
class VHArchive:
    ...
    def pack(files, output_path, password=None, use_crc=True):
        ...
        for name, content in files:
            if isinstance(content, str): content = content.encode("utf-8")
            compressed = VHCompressor.compress(bytes(content))
            crc = zlib.crc32(content) & 0xFFFFFFFF if use_crc else 0
            payload = compressed
            nonce_bytes, tag_bytes = b"", b""
            if key:
    ...
    @staticmethod
    def list_files(archive_path, password=None):
        """Return list of (name, orig_size, comp_size, has_crc, is_encrypted, crc_value)."""
        with open(archive_path, "rb") as f:
            data = f.read()
        hdr = _read_archive_header(data, VHCompressor.MAGIC, VHArchive.V2_MAGIC)
        if not hdr: return None
